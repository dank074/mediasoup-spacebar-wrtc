import { ClientEmitter, Codec, RtpHeader, SSRCs, VideoStream, WebRtcClient } from "spacebar-webrtc-types";
import { VoiceRoom } from "./VoiceRoom";
import EventEmitter from "node:events";
import { Consumer, Producer, RtpCodecCapability, RtpHeaderExtension, RtpHeaderExtensionUri, Transport } from "mediasoup/node/lib/types";
...
export class MediasoupWebRtcClient implements WebRtcClient<any> {
    ...
    getOutgoingStreamSSRCsForUser(user_id: string): SSRCs {
		...
		const audioConsumer = this.consumers?.find(consumer => consumer.producerId === audioProducerId)
		const videoConsumer = this.consumers?.find(consumer => consumer.producerId === videoProducerId)

		return { 
			audio_ssrc: audioConsumer?.rtpParameters.encodings?.find((y) => y !== undefined)?.ssrc,
			video_ssrc: videoConsumer?.rtpParameters.encodings?.find((y) => y !== undefined)?.ssrc,
			rtx_ssrc: videoConsumer?.rtpParameters.encodings?.find((y) => y !== undefined)?.rtx?.ssrc
		}
	}
    ...
    async publishTrack(type: "audio" | "video", ssrc: SSRCs):Promise<void> {
		...
		if(type === "audio" && !this.isProducingAudio()) {
			this.audioProducer = await this.transport.produce({
				kind: "audio",
				rtpParameters: {
					codecs: this.codecCapabilities?.filter(codec => codec.kind === "audio").map(codec => {
						const {mimeType, clockRate, channels, rtcpFeedback, parameters } = codec;

						return { mimeType, clockRate, channels, rtcpFeedback, parameters, payloadType: codec.preferredPayloadType ?? 111 }
					}) ?? [],
					encodings: [
						{
		...
		if(type === "video" && !this.isProducingVideo()) {
			this.videoProducer = await this.transport.produce({
				kind: "video",
				rtpParameters: {
					codecs: this.codecCapabilities?.filter(codec => codec.kind === "video").map(codec => {
						const {mimeType, clockRate, channels, rtcpFeedback, parameters } = codec;

						return { mimeType, clockRate, channels, rtcpFeedback, parameters, payloadType: codec.preferredPayloadType ?? 102 }
					}) ?? [],
					encodings: [
						{
					...
					headerExtensions: this.headerExtensions?.filter(header => header.uri === "http://www.webrtc.org/experiments/rtp-hdrext/playout-delay").map((header) => {
					  return { id: header.id, uri: header.uri as RtpHeaderExtensionUri };
					}),
				},
				paused: false,
    ...
    stopPublishingTrack(type: "audio" | "video"): void {
		...
		for (const client of this.room.clients.values()) {
			...
			consumers?.forEach((consumer) => {
				...
				if(typeof index === 'number' && index != -1) client.consumers?.splice(index, 1);
			});
		}
    ...
    async subscribeToTrack(user_id: string, type: "audio" | "video"): Promise<void> {
		...
		const consumer = await this.transport.consume({
			...
			rtpCapabilities: {
				...
				headerExtensions: this.headerExtensions?.map(header => {
					return { preferredId: header.id, uri: header.uri as RtpHeaderExtensionUri, kind: type}
				}) ?? [],
			},
			paused: type === "video",
			appData: {
    ...
    unSubscribeFromTrack(user_id: string, type: "audio" | "video"): void {
		...
		if(typeof index === 'number' && index != -1) this.consumers?.splice(index, 1);
	}
